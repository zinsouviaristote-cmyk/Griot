...
export function ChoiceStep() {
  ...
  function trackFor(attempt: SongAttempt): PlayerTrack {
    return {
      id: attempt.id,
      title: data.recipientFirstName || "Votre chanson",
      subtitle: `Essai ${attempt.index}`,
      occasion,
  ...
  return (
    <div>
      ...
      <div className="mt-6 rounded-card border border-dashed border-border bg-page p-4">
        {notesBalance > 0 ? (
          <>
            <p className="text-sm font-medium text-ink">Pas encore convaincu ?</p>
            <p className="mt-0.5 text-xs text-ink-muted">
              Réessayer utilisera 1 Note — il vous en restera {notesBalance - 1}.
            </p>
            <button
              type="button"
              onClick={handleRetry}
          ...
          <>
            <p className="flex items-center gap-1.5 text-sm font-medium text-ink">
              ...
              Solde de Notes épuisé
            </p>
            <p className="mt-0.5 text-xs text-ink-muted">
              Gardez l&apos;essai qui vous plaît déjà, téléchargeable dès maintenant — ou rechargez pour en tenter
              un de plus.
            </p>
            <div className="mt-3 grid gap-2 sm:grid-cols-3">
              {CREDIT_PACKS.map((pack) => (
                <div key={pack.id} className="rounded-control border border-border bg-surface p-3 text-center">
                  <p className="font-display text-lg font-bold text-ink">{packNotes(pack)} Notes</p>
                  <p className="text-label-sm text-ink-muted">{formatPackEquivalence(pack)}</p>
